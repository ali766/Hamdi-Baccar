...
function AdminDashboard({ back, students, setStudents, lessons, setLessons, progress, adminPass, setAdminPass, lang, setLang }) {
  ...
  return (
    <Board lang={lang}>
      ...
      {tab === "lessons" && <LessonsTab t={t} lessons={lessons} setLessons={setLessons} />}
...
function StudentsTab({ t, students, setStudents }) {
  ...
  const add = (e) => {
    ...
    setStudents([...students, { id: uid(), name: name.trim(), phone: normPhone(phone), createdAt: new Date().toISOString() }]);
    setName("");
    setPhone("");
  };
...
function LessonsTab({ t, lessons, setLessons }) {
  const [form, setForm] = useState({ title: "", category: "", desc: "", url: "" });
  const add = (e) => {
    e.preventDefault();
    if (!form.title.trim()) return;
    setLessons([...lessons, { id: uid(), ...form, category: form.category.trim() || t.noCategory, createdAt: new Date().toISOString() }]);
    setForm({ title: "", category: "", desc: "", url: "" });
  };
  ...
  return (
    <div>
      <form onSubmit={add} style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24, border: `1.5px dashed ${COLORS.chalkDim}`, borderRadius: 10, padding: 16 }}>
        ...
        <ChalkInput label={t.lessonUrl} value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} placeholder="https://..." dir="ltr" />
...
function ProgressTab({ t, lang, students, lessons, progress }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {students.map((s) => {
        const stats = studentStats(s.id, lessons, progress);
...
function StudentOtp({ back, student, onVerified, lang, setLang }) {
  const t = T[lang];
  const [sentCode] = useState(() => genOtp());
  const [entered, setEntered] = useState("");
  const [err, setErr] = useState("");

  const submit = (e) => {
    e.preventDefault();
    if (entered.trim() === sentCode) onVerified();
  ...
  return (
    <Board lang={lang}>
      <TopBar back={back} label={t.back} lang={lang} setLang={setLang} />
      <Title lang={lang} sub={t.codeSentTo(student.name)}>{t.confirmLogin}</Title>
      <div style={{ maxWidth: 340, margin: "0 auto 18px", border: `1.5px dashed ${COLORS.chalkYellow}`, borderRadius: 10, padding: "12px 16px", color: COLORS.chalkYellow, fontSize: 13, lineHeight: 1.8, textAlign: "center" }}>
        {t.demoNote}
        <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: 4, marginTop: 6, direction: "ltr" }}>{sentCode}</div>
      </div>
      <form onSubmit={submit} style={{ maxWidth: 340, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16 }}>
...
function StudentDashboard({ back, student, lessons, progress, setProgress, lang, setLang }) {
  const t = T[lang];
  const stats = studentStats(student.id, lessons, progress);
  const grouped = useMemo(() => {
    const g = {};
    lessons.forEach((l) => { g[l.category] = g[l.category] || []; g[l.category].push(l); });
    return g;
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [s, l, p, a] = await Promise.all([
        loadShared("lms-students", []),
        loadShared("lms-lessons", []),
        loadShared("lms-progress", {}),
        loadShared("lms-admin-pass", "2580"),
      ]);
      setStudentsState(s);
      setLessonsState(l);
      setProgressState(p);
  ...
  const setStudents = useCallback((v) => { setStudentsState(v); saveShared("lms-students", v); }, []);
  const setLessons = useCallback((v) => { setLessonsState(v); saveShared("lms-lessons", v); }, []);
  const setProgress = useCallback((v) => { setProgressState(v); saveShared("lms-progress", v); }, []);
  const setAdminPass = useCallback((v) => { setAdminPassState(v); saveShared("lms-admin-pass", v); }, []);

  if (!loaded) {
    return (
      <Board lang={lang}>
        <Title lang={lang}>{T[lang].brand}</Title>
        <div style={{ textAlign: "center", color: COLORS.chalkDim }}>{T[lang].loading}</div>
      </Board>
    );
  }
